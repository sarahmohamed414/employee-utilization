"""
Employee Utilization Dashboard with MCP Integration

This is your existing dashboard enhanced with MCP (Model Context Protocol)
for dynamic data fetching from Microsoft Lists.
"""
...
try:
    from integrate_mcp import integrate_mcp_with_dashboard
    MCP_AVAILABLE = True
except ImportError:
    MCP_AVAILABLE = False
    st.warning("⚠️ MCP integration not available. Install dependencies: pip install -r requirements.txt")
...
def load_and_process_data(csv_path):
    ...
    melted_df['Email'] = 'N/A'
    melted_df['Id'] = range(len(melted_df))
...
def create_weekly_tables(melted_df):
    ...
    for week in available_weeks:
        ...
        weekly_util = week_data.groupby('Name').agg({
            'Actual Hrs': 'sum',
            'Project ID': lambda x: list(set(x)),
            'Overtime': 'sum'
        }).reset_index()
...
NORMAL_UTILIZATION_MIN = 0.8
NORMAL_UTILIZATION_MAX = 1.0
...
data_source = st.sidebar.radio(
    "Choose data source:",
    ["Static CSV File", "Dynamic Microsoft Lists (MCP)"],
    help="Select whether to use static CSV data or dynamic data from Microsoft Lists"
)

# Load data based on selection
if data_source == "Static CSV File":
    csv_path = 'Anonymized_Employee_Utilization.csv'
    melted_df = load_and_process_data(csv_path)
    st.info("📁 Using static CSV data")
else:
    # Try to get dynamic data from MCP
    if MCP_AVAILABLE:
        dynamic_data = integrate_mcp_with_dashboard()
        if dynamic_data is not None:
            # Process dynamic data (assuming it's in the same format)
            melted_df = dynamic_data
            st.success("🔄 Using live data from Microsoft Lists")
        else:
            # Fallback to static data
            csv_path = 'Anonymized_Employee_Utilization.csv'
            melted_df = load_and_process_data(csv_path)
            st.warning("⚠️ MCP not configured, using static CSV data")
    else:
        # Fallback to static data
        csv_path = 'Anonymized_Employee_Utilization.csv'
        melted_df = load_and_process_data(csv_path)
        st.warning("⚠️ MCP not available, using static CSV data")
...
st.header("AI Assistant for Utilization Analysis")

# Generate context for AI
context = generate_data_context(melted_df, weekly_tables)
...
user_query = st.text_input(
    "Ask a question about the utilization data:",
    key="query_input",
    placeholder="e.g., 'Who are the most utilized employees?' or 'Show me under-utilized employees in Week 5'"
)
...
# Graph-Based Utilization Analysis Section
st.header("Graph-Based Utilization Analysis")

# Initialize Neo4j connection only when needed
@st.cache_resource
...
util_graph = get_neo4j_connection()
if util_graph:
    ...
    st.info("You can still use the dashboard for basic analysis without the graph features.")
